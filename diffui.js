...
document.addEventListener("DOMContentLoaded", function () {
    ...
    setTheme(parseInt(window.localStorage.getItem('SchematicTheme')) || 1);
    document.getElementById('zoomcontrols').style.display = window.localStorage.getItem('ShowZoomControls') == 'shown' ? 'inline' : 'none';
    ...
    document.getElementById('tooltiplink').addEventListener('click', function () {
        copyToClipboard(window.location.href.split('#')[0] + '#' + currentPage + ',' + document.getElementById('tooltipname').textContent, 'link');
    });
    ...
    document.getElementById('settingsbutton').addEventListener('click', function () {
        ...
        if (window.localStorage.getItem('ShowZoomControls') == 'shown') {
        ...
        let toption = document.getElementById(
            'toption-' + (window.localStorage.getItem('SchematicTheme') || 1));
        if (toption) {
    ...
    document.getElementById('applysettings').addEventListener('click', function () {
        window.localStorage.setItem('ShowZoomControls', document.getElementById('zoomcontrolcheckbox').checked ? 'shown' : 'hidden');
        ...
        setTheme(parseInt(selectedTheme.getAttribute('value')));
        toggleDialog(dialog, false);
    });
        ...
        if (Object.keys(unresolvedConflictPages).length) {
            ...
            document.getElementById('unresolvedWarning').innerHTML = 'Some conflicts do not have any changes selected:<ul>'
                + (Object.keys(unresolvedConflictPages).map(k =>
                    `<li>${unresolvedConflictPages[k]} unresolved in ${getPageName(k)}</li>`)).join('')
    ...
    if (uiMode > 1) {
        ...
        document.getElementById('highlight-changes-button').addEventListener('click', function () {
            ...
            for (let diffPair of data.diffs[currentPage])
...
let colors = {
    'r': ['red', 'red', 'black', 'white', '#DC322F', '#DC322F'],
    'g': ['green', 'lime', 'black', 'white', '#859900', '#859900'],
    'u': ['blue', 'blue', 'black', 'white', '#268BD2', '#268BD2'],
    'y': ['goldenrod', 'yellow', 'black', 'white', '#B58900', '#B58900'],
    'o': ['orange', 'orange', 'black', 'white', '#002B36', '#FDF6E3'],
    'k': ['black', 'white', 'black', 'white', '#CB4B16', '#CB4B16'],
    'n': ['salmon', 'salmon', 'black', 'white', '#CB4B16', '#CB4B16'],
    'v': ['darkviolet', 'violet', 'black', 'white', '#6C71C4', '#6C71C4'],
    'x': ['brown', 'brown', 'black', 'white', '#CB4B16', '#CB4B16'],
    'b': ['dodgerblue', 'skyblue', 'black', 'white', '#268BD2', '#268BD2'],
    'w': ['black', 'white', 'black', 'white', '#002B36', '#EEE8D5'],
    'h': ['peachpuff', 'peachpuff', 'black', 'white', '#6C71C4', '#6C71C4'],
    'p': ['deeppink', 'pink', 'black', 'white', '#D33682', '#D33682'],
    'l': ['purple', 'purple', 'black', 'white', '#6C71C4', '#6C71C4'],
    'q': ['aqua', 'aqua', 'black', 'white', '#2AA198', '#2AA198'],
    'a': ['gray', 'gray', 'black', 'white', '#839496', '#839496'],
    'c': ['white', 'black', 'white', 'black', '#FDF6E3', '#002B36']
};
...
function setTheme(selection, target) {
    if (!target) {
        window.localStorage.setItem('SchematicTheme', selection);
    }
    for (v in colors) {
        (target || document.body).style.setProperty('--' + v, colors[v][selection - 1]);
    }
...
function fillPageList(pages) {
    ...
    for (let p of pages) {
        ...
        spanElem.innerHTML = '<code>' + p[0].slice(4) + '&nbsp;</code>' + p[1];
        spanElem.id = p[0] + '_link';
...
function injectPage(svgID) {
    ...
    let page = data.pages.filter(p => p[0] == svgID)[0];
    let svg = null;
    if (currentPage !== svgID || !currentPage) {
        currentPage = svgID;
        ...
        svgPage.viewBoxOriginal = page[2];
        ...
        let svgIndex = getPageIndex(svgID);
        ...
        let yOffset = 0;
        let pageOffset = 0;
        for (let i = Math.max(0, svgIndex - 2); i < Math.min(data.pages.length, svgIndex + 3); i++) {
            let targetSvg = ghostSvg;
            if (i == svgIndex) {
                pageOffset = yOffset;
                yOffset += page[2][3] + pageSeparation;
                continue;
            }
            yOffset += addGhostPage(ghostSvg, data.pages[i], yOffset, i > svgIndex, svgPage.viewBoxOriginal[2]);
        }
...
function applyAnimationColorWorkaround() {
    let themeIndex = (parseInt(window.localStorage.getItem('SchematicTheme')) || 1) - 1;
    // workaround the fact that we cannot animate css variables.
    document.querySelectorAll('animate[fromvar]').forEach(animElem => {
        let fromVar = animElem.getAttribute('fromvar').match(/(?<=var\(--)[a-z](?=\))/);
        let toVar = animElem.getAttribute('tovar').match(/(?<=var\(--)[a-z](?=\))/);
        if (fromVar && toVar) {
            animElem.setAttribute('from', colors[fromVar][themeIndex]);
            animElem.setAttribute('to', colors[toVar][themeIndex]);
        }
    });
}

function addGhostPage(ghostSvg, page, yOffset, pageBelow, activeWidth) {
    ...
    ghostText.setAttribute('y', yOffset + (pageBelow ? 300 : viewBox[3] - 150));
    ghostText.setAttribute('font-size', 150);
    ghostText.innerHTML = `${arrowChar} ${page[1]} ${arrowChar}`;
...
function initializeCheckModel() {
    for (let page of data.pages) {
        ...
        for (let diffs of data.diffs[page[0]]) {
            // do the raw indexing
            for (let i of [0, 1]) {
        ...
        if (pageHasConflict) {
            document.getElementById(page[0] + '_link').classList.add('conflictpagelink');
        }
    }
}
...
function initializeExclusions() {
    for (let page of data.pages) {
        for (let diffPair of data.diffs[page[0]]) {
...
function fillDiffTable() {
    ...
    let diffIndex = 0;
    let tbody = '';
    for (let diffs of data.diffs[currentPage]) {
...
function setMultipleDiffChecks() {
    ...
    for (let diffPair of data.diffs[currentPage].filter(dp => Math.max(dp[0].length, dp[1].length) > 1)) {
        for (let i of [0, 1]) {
...
function checkAll(diffPairIndex, operation) {
    let rowCtr = 0;
    for (let i in data.diffs[currentPage]) {
        let diffPair = data.diffs[currentPage][i];
...
function filterChanges(filter) {
    ...
    let rowCtr = 0;
    for (let diff of data.diffs[currentPage]) {
...
function filterSearch(filter) {
    ...
    for (let comp in data.comps) {
        ...
        for (let inst of data.comps[comp]) {
            if (!uniqueProjs.filter(c => {
                return c.mpn == inst[3] && c.pn == inst[2];
            }).length) {
                uniqueProjs.push({
                    type: 'component',
                    pages: data.comps[comp].filter(i => i[3] == inst[3] && i[2] == inst[2]).map(i => i[0]),
                    mpn: inst[3],
                    pn: inst[2],
                    display: comp
                });
        ...
        for (let proj of uniqueProjs) {
            let projMatchDistance = matchesComponent(filter, proj);
            if (projMatchDistance[0] < matchThreshold) {
                proj.substring = projMatchDistance[1];
                matchMatrix[projMatchDistance[0]].push(proj);
            }
        }
    }
...
function cyclePage(delta, retainPan, mouseEvent, leftoverPanY) {
    ...
    let pageIDs = data.pages.map(p => p[0]);
    let currentIndex = pageIDs.indexOf(currentPage);
    if (currentIndex + delta >= 0 && currentIndex + delta < data.pages.length) {
        pushHash(pageIDs[currentIndex + delta]);
        window.onpopstate();
...
function getPages(pList, ref, color, zebra) {
    ...
    for (let page in pCounts) {
        let p = parseInt(page);
        rawHTML += `<div${zebra && pCounter % 2 ? ' style="background-color:rgba(0,0,0,0.4)"' : ''}><a class="itempagelink" style="color:${color}" href="${'#' + (getPageIndexID(p) + ',' + ref)}" onclick="clickedPageLink(this, event); return false">${getPageName(p) + (pCounts[p] > 1 ? ' (' + pCounts[p] + ')' : '')}</a></div>`;
        pCounter++;
    }
    return rawHTML;
}
...
function matchesComponent(term, component) {
    let match = [matchesTerm(term, component.mpn), "MPN: " + component.mpn];

    let rdMatch = matchesTerm(term, component.display);
    if (rdMatch < match[0]) {
        match = [rdMatch, "RefDes: " + component.display];
    }

    let pnMatch = matchesTerm(term, component.pn);
    if (pnMatch < match[0]) {
        match = [pnMatch, "PN: " + component.pn];
    }
...
function displayTooltip(elem, target, fix) {
    ...
    //show pn link
    if (elem.type === 'component' && elem.indexed[2]) {
        document.getElementById('pndiv').style.display = null;
        document.getElementById('pnlink').textContent = 'G' + elem.indexed[2];
        let href = 'urlToPN' + elem.indexed[2];
        document.getElementById('pnlink').setAttribute('href', href);
        document.getElementById('pnlink').setAttribute('onclick', 'openurl("' + href + '"); return false;');
    } else {
...
function getTooltipLinks(elem) {
    if (elem.type === 'net') {
        ...
        return getPages(data.comps[elem.name].map(e => e[0]), elem.name, "blue", false);
    } else {
        return [];
    }
}
...
function getIndexedElem(closest, name, typ) {
    switch (typ) {
        case 'component':
            ...
            if (path && name) {
                return data.comps[name].find(i => i[1] == path);
            }
            return null;
        case 'net':
...
function getElemName(closest, typ) {
    switch (typ) {
        case 'component':
            return getLocation(currentPage, closest.getAttribute('p'));
        case 'net':
...
/** Returns location for a given path (assuming on currentPage) */
function getLocation(page, path) {
    // create a page,path -> refdes index if not already created
    if (!data.compsByPath) {
        data.compsByPath = {};
        for (let loc in data.comps) {
            for (let inst of data.comps[loc]) {
                data.compsByPath[inst[0]] || (data.compsByPath[inst[0]] = {});
                data.compsByPath[inst[0]][inst[1]] = loc;
            }
        }
    }
    let pageIndex = getPageIndex(page);
...
function openurl(url) {
    if (document.location.hostname !== 'localhost' || document.location.pathname !== '/') {
...
window.onpopstate = function (evt) {
    currentPage = '';
    let refdes = decodeURI(window.location.hash.replace('#', '')).toUpperCase();
    let page = '';
    if (refdes.indexOf(',') != -1) {
        page = refdes.split(',')[0].toLowerCase();
        refdes = refdes.split(',')[1];
    } else {
        let comp = data.comps[refdes.split('.')[0].toUpperCase()] || '';
        if (comp) {
            page = getPageIndexID(comp[0][0]);
        }
    }

    if (/^(page[1-9]\d*)/.test(refdes.toLowerCase()) && data.pages.filter(p => p[0] == refdes.toLowerCase()).length) {
        injectPage(refdes.toLowerCase());
    } else if (/^(page[1-9]\d*)/.test(page.toLowerCase()) && data.pages.filter(p => p[0] == page.toLowerCase()).length) {
        injectPage(page.toLowerCase());
    }

    displayTooltip(false);

    if (!refdes && !page) {
        window.location.hash = '#' + getPageIndexID(0);
        return;
    }

    // Match components
    let instancesMatched = [];
    if (data.comps[refdes]) {
        let pathsFromRefdes = data.comps[refdes].filter(i => i[0] == getPageIndex(page)).map(i => i[1]);
    ...
    for (let prop of svgPage.getElementsByTagName('text')) {
        ...
        if (elem.type === 'net' && elem.name === refdes) {
    ...
    for (let txt of getSymbolTexts()) {
        if (matchesTerm(txt[0].textContent, refdes) === 0) {
    ...
    for (let prop of Array.from(svgPage.getElementsByTagName('text')).filter(p =>
        ...
        if (matchesTerm(prop.textContent, refdes) === 0) {
            // Highlight the prop
...
function getBounds(elems) {
    let clientRects = elems.map(e => e.getBoundingClientRect());
    let bounds = {
...
function panToElems(targetElems) {
    ...
    currentPanZoom.zoom(Math.min(5, ...[
        0.8 * (currentPanZoom.getSizes().width + widthOffset) / (elemBounds.right - elemBounds.left),
        0.8 * currentPanZoom.getSizes().height / (elemBounds.bottom - elemBounds.top)
    ]));
...
function getPageIndexID(pageName) {
    if (typeof (pageName) === 'number') {
        return data.pages[pageName][0];
    }
    for (let p of data.pages) {
        if (p[1] == pageName) {
            return p[0];
...
function getPageIndex(pageID) {
    return data.pages.map(p => p[0]).indexOf(pageID);
}
...
function cycleInstance(direction, elem, closest) {
    ...
    const matches = getMatches(closest);
    const current_elem_index = matches.indexOf(elem);
    let target = elem;

    if (closest.type === 'net') {
        pageList = data.nets[closest.name];
    } else if (closest.type === 'component') {
        pageList = data.comps[closest.name].map(i => i[0]);
    } else {
        return;
    }
    if (direction) {
    ...
    let target_href = currentPage + "," + target.id;

    if (target.isSameNode(elem) && pageList.length > 1) {
        // Need to navigate to a different page
        let cycleIndex = 0;
        for (let i = 0; i < pageList.length; ++i) {
            let pagenum = getPageIndexID(pageList[i]);
        ...
        let pagenum = getPageIndexID(pageList[cycleIndex]);
        injectPage(pagenum);
        const second_matches = getMatches(closest);
        ...
        if (direction) {
            target = second_matches[0];
        } else {
            target = second_matches[second_matches.length - 1];
        }
        if (target.tagName === "text") {
            // Net targets do not have ids that can be referenced add one temporarily to follow link
            target.id = closest.name;
        }
        target_href = pagenum + "," + target.id;
    }
    highlight([target], true, true, true);
