...
def to_png(f):
  ...
  if bpp <= 8:
    color_table = tuple(
      f[x + 2 : x - 1 : -1]
      for x in range(14 + header_size, 14 + header_size + num_colors * 4, 4)
    )
  elif bpp == 32:
    ...
    for y in range(h - 1, -1, -1):
      ...
      if bpp == 1:
        ...
        for remainder in range(w % 4):
          row += color_table[
            (data[start + w // 4] >> (6 - 2 * remainder)) & 0x3
          ]
        yield row
      elif bpp == 4:
        row = list(
          itertools.chain.from_iterable(
            map(
              lambda a: color_table[a >> 4] + color_table[a & 0xF],
        ...
        yield list(
          itertools.chain.from_iterable(
            map(
              ...
              data[start : start + w * 2 : 2],
              data[start + 1 : start + w * 2 : 2],
            )
          )
        )
  ...
  return png.encode(
    rows=getrows(), width=w, height=h, has_alpha=has_alpha, bitdepth=8
  )
