...
class Page(object):
  def __init__(self, name, page, instances, safediffs, conflicts,
               proj, variables, uidtable, symbols, worksheet):
    self.sch = page
    ...
    page.fillsvg(self.svg, diffs,
                 Drawable.DRAW_STAGE_COMMON_BG,
                 context=context)
    ...
    for path, sheet in instances:
      ...
      page.fillsvg(self.svg, diffs,
                   Drawable.DRAW_STAGE_PAGE_SPECIFIC,
                   context=pgcontext)
      self.svg.gend()
    self.svg.metadata_context = None
    # Common foreground elements
    page.fillsvg(self.svg, diffs,
                 Drawable.DRAW_STAGE_COMMON_FG,
                 context=context)
  ...
  def _update_index(self):
    ...
    def add_to_index(toc, depth=0):
      for inst in toc:
        ...
        self.schematic_index['pages'].append({
            ...
            "contentbox": tuple(map(int,ui_page.svg.get_viewbox(with_wks=False))),
            })
    ...
    for i, (ui_page, page) in enumerate(
      ...
      for uuid, pin_num, pin_name in ui_page.svg.pin_text:
        if uuid in (None, instance):
          self.schematic_index['pins'].setdefault(pin_name, []).append([i, pin_num])

  def addpage(self, name, page, instances, safediffs, conflicts):
    p = Page(name, page, instances, safediffs, conflicts,
      self._proj, self._variables, self._uidtable, self._symbols,
      self._worksheet)
  ...
  def genhtml(self):
    ...
    if self._mode >= DiffUI.MODE_DIFF:
      html.append("let diffIcon = '%s';" %
          self._icon('%s.svg' % DiffUI.MODE_ICONS[self._mode]))
    html.append("let feedbackURL = '%s';" % (
      self._variables and self._variables.resolve(self._variables.GLOBAL, "feedbackURL") or ""))
    html.append("let themeDefault = '%s';" % themes.themes()[0][0])
    html.append("let themeBW = '%s';" %
        next(t[0] for t in themes.themes() if "black" in t[0].lower() and "white" in t[0].lower()))
...
def main(argv):
  ...
  for i in range(0, len(paths), count):
    schs = [sch_mod.kicad_sch(open(p, 'rb'), p) for p in paths[i:i+min(count,3)]]
  ...
  for i in range(len(paths)//count):
    ...
    base_schs[i].write(open(paths[i*4+3], 'wb') if count == 4 else sys.stdout.buffer)
