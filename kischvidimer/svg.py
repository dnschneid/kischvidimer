...
class Svg(object):
  ...
  def __init__(self, bgcolor='SCHEMATIC_BACKGROUND', header=True,
               autoAnimate=(2, 1), mirror_text=False,
               theme=None):
    """Preps an empty SVG.
  ...
  def _update_bounds(self, pos, pos2=None, thick=0):
    ...
    if self._bounds is None:
      self._bounds = pos*2
    thick = float(thick)
    self._bounds = (min(self._bounds[0], pos[0] - thick/2),
  ...
  def _prune(self, force=False):
    ...
    return (self.prune or force) and any(t[0][0] == 'hide' for t in self._transforms if t)
  ...
  def attr_opacity(self, cnt, i, name='opacity'):
    ...
    return self.attr(name,
        [(1*(i==0), None), (1*(i>0), c)], 1, convert=False)
  ...
  def gstart(self, pos=None, rotate=None, mirror=False, hidden=False,
             path=None, tag=None):
    ...
    if not prune and (len(pos) > 1 or pos[0][0] != (0, 0)):
      transform.append(('translate',
                       float(pos[0][0][0]), float(self.y(pos[0][0][1]))))
      self.add(['<g']
      + Svg._tagattr(tag)
      + self.attr('p', path, '')
      + self.attr('translate', [((Svg.tounit(p[0]),Svg.tounit(self.y(p[1]))), c) for p,c in pos])
  ...
  def line(self, p1=(0,0), p2=None, color='wire', thick='wire', pattern=None):
    ...
    pattern = [(Svg.pattern(pattern.get(i)[0], thick.get(i)[0]),
  ...
  def rect(self, pos=(0,0), width=None, height=None, end=None,
           color='notes', fill=None, thick='wire', pattern=None, tag=None):
    pos = Param.ify(pos, (0, 0))
    if end is not None:
      end = Param.ify(end)
      classes = [Svg.classunion(end.get(i), pos.get(i))
                 for i in range(max(map(len, (pos, end))))]
      width = [(abs(end.get(i)[0][0]-pos.get(i)[0][0]),c)
               for i,c in enumerate(classes)]
      height = [(abs(end.get(i)[0][1]-pos.get(i)[0][1]),c)
                for i,c in enumerate(classes)]
      x = [(min(end.get(i)[0][0],pos.get(i)[0][0]),c)
           for i,c in enumerate(classes)]
      y = [(min(self.y(end.get(i)[0][1]),self.y(pos.get(i)[0][1])),c)
           for i,c in enumerate(classes)]
    else:
  ...
  def circle(self, pos=(0,0), radius=None, color='notes', fill=None,
             thick='wire', pattern=None):
    pos = Param.ify(pos, (0, 0))
  ...
  def arc(self, start, stop, mid=None, radius=None, largearc=None,
          color='notes', fill=None, thick='wire', pattern=None):
    start = Param.ify(start)
    stop = Param.ify(stop)
    center = None
    if mid is not None:
      mid = Param.ify(mid)
      crlas = [Svg.c_r_la(start.get(i)[0], mid.get(i)[0], stop.get(i)[0]) for
               i in range(max(map(len, (start, stop, mid))))]
      center = Param.ify([(
        crlas[i][0],
        Svg.classunion(start.get(i), stop.get(i), mid.get(i)))
    ...
    pattern = [(Svg.pattern(pattern.get(i)[0], thick.get(i)[0]),
                Svg.classunion(pattern.get(i), thick.get(i)))
               for i in range(max(map(len, (pattern, thick))))]
    d = [('M %s %s A %s %s 0 %d %d %s %s' % (
          Svg.tounit(start.get(i)[0][0]), Svg.tounit(self.y(start.get(i)[0][1])),
          Svg.tounit(radius.get(i)[0]), Svg.tounit(radius.get(i)[0]),
          1*largearc.get(i)[0],
          0.5+self.y(0.5),  # sweep direction is opposite if y is flipped
          Svg.tounit(stop.get(i)[0][0]), Svg.tounit(self.y(stop.get(i)[0][1])))
          + ('L %s %s Z' % (Svg.tounit(center.get(i)[0][0]),
                            Svg.tounit(self.y(center.get(i)[0][1])))
             if center and fill.get(i)[0] != 'none' else ''),
          Svg.classunion(start.get(i), stop.get(i), radius.get(i),
                         largearc.get(i), fill.get(i)))
  ...
  def polyline(self, xys, color='notes', fill=None, thick='wire',
               pattern=None):
    """ Renders a polyline.
    ...
    if not isinstance(xys, Param):
      ...
      if isinstance(xys[0][0], (list, tuple)):
        ...
        xys = [(xys, None)]
    d = [("M %s %s %s" % (Svg.tounit(pts[0][0]), Svg.tounit(self.y(pts[0][1])),
          " ".join("L %s %s" % (Svg.tounit(pt[0]), Svg.tounit(self.y(pt[1])))
                   for pt in pts[1:])),c)
         for pts,c in xys]
    # FIXME: don't emit anything if color and fill are none?
    color, opacity = self._color(color, 'notes')
  ...
  def image(self, data, pos=(0,0), scale=1):
    ...
    for i in range(len(data)):
      ...
      self._update_bounds(pos[i][0],
          (pos[i][0][0]+width[i][0], pos[i][0][1]+height[i][0]))
      ...
      + self.attr('image-rendering', [
        ('pixelated' if s >= self.IMAGE_PIXEL_SCALE_THRESHOLD else '',c)
        for s,c in scale], '', i)
      + self.attr_opacity(data, i=i)
      ).nocontents()

  def text(self, text, prop='', pos=(0,0), size='100%', color='notes',
           justify='middle', vjustify='middle', rotate=None, hidden=None,
           url=None, tag=None):
    needsgroup = False
    rotate = Param.ify(rotate, 0)
    hidden = Param.ify(hidden, False)
    ...
    url = Param.ify(url)
    if (len(rotate) > 1 or rotate[0][0] or
        len(hidden) > 1 or hidden[0][0] or
        len(text) > 1 or '\n' in text[0][0] or
        len(url) > 1 or
        self._mirror_text):
      needsgroup = True
    ...
    textpos = (textpos[0], textpos[1] + self.y(size[0][0] * linecount * vjust[0][0][0]))
    # Reference: gr_text.cpp: reg=size/8, demibold=size/6, bold=size/5
    self._update_bounds(textpos,
    ...
    + Svg._tagattr(tag)
    + (['prop="%s"' % prop] if prop and isinstance(prop, str) else [])
    ).hascontents()
    ...
    if prop and isinstance(prop, int) and prop < Svg.PROP_LABEL:
      ...
      if not self.pin_text or len(self.pin_text[-1]) == 3:
        ...
        self.pin_text[-1] = (self.pin_text[-1][0], pintext, self.pin_text[-1][1])
    elif not tag and prop not in self.GENERIC_IGNORE and not self._prune(force=True):
      self.generic_text.append((self.metadata_context, '\n'.join(t for t,_ in text)))
    for i in range(len(text)):
      if url.get(i)[0]:
        self.add(["<a href='%s'%s>" % (url.get(i)[0],
                  " target='_blank'"*(not url.get(i)[0].startswith('#')))],
                  extend=True)
      opacity = ([] if len(text) == 1 else
        self.attr_opacity(text, i=i, name='fill-opacity'))
      baseline = self.attr('dominant-baseline', [(vj[1],c) for vj,c in vjust],
                           'baseline')
      splittext = text[i][0].split('\n')
      for lineno, line in enumerate(splittext):
        colcount = 0
        for colno, t in enumerate(line.split('\t')):
          encoded = Svg.encode(t or '') or ' '
          yattr = self.attr('y',
              [('%.1gem' % ((len(splittext)-1)*(vj[0]-1)), c) for vj,c in vjust],
              '-0em') if len(splittext) > 1 and not lineno else []
          self.add(['<tspan']
          + ['x="0"', 'dy="1em"'] * (lineno > 0) * (colno == 0)
          + ['x="%gex"' % ((colcount//4+1)*4*(1+Svg.FONT_SPACING))] * (colno > 0)
          + yattr
          + baseline
          + opacity
          , extend=True  # stray whitespace in <text> causes misalignment
          ).hascontents('%s</tspan>' % encoded)
          colcount += len(t)
      if url.get(i)[0]:
  ...
  def instantiate(self, draw, lib, lib_id, unit=1, variant=1, context=None):
    ...
    name = [('symbol:%x:%d:%d:%d%s:%x' % (
      lib.get(i)[0].sym_hash(lib_id.get(i)[0]),
      unit.get(i)[0], variant.get(i)[0],
      self._rotate_state(i)//90,
    ...
    for i in range(len(name)):
      ...
      symsvg = Svg(
          self.bgcolor, header=False, autoAnimate=False,
          mirror_text=self._mirror_state(i))
      symsvg.push_invert_y()
      symsvg.colormap = self.colormap
      sym.fillsvg(symsvg, [], draw, context or tuple(),
                  unit=int(params[2]), variant=int(params[3]))
      symsvg.pop_invert_y()
  ...
  def instantiate_worksheet(self, draw, context, worksheet=None):
    ...
    name = [(None if wks.get(i)[0] is None else
             'wks:%x' % (wks.get(i)[0].wks_hash(context.get(i)[0])),
    ...
    if draw & Drawable.DRAW_WKS_PG:
      for i in range(len(name)):
        if wks.get(i)[0]:
          wks.get(i)[0].fillsvg(self, [], Drawable.DRAW_WKS_PG, context.get(i)[0])
  ...
  def _instantiate(self, name):
    for i in range(len(name)):
      ...
      if symsvg.data:
        ...
        self.generic_text.extend((self.metadata_context,t) for _,t in symsvg.generic_text)
        self.pin_text.extend((self.metadata_context,)+t[1:] for t in symsvg.pin_text)
        bounds = symsvg._bounds
        self._update_bounds((bounds[0], self.y(bounds[1])), (bounds[2], self.y(bounds[3])))
    return True
  ...
  def imagedata(data, convert_all=False):
    ...
    for typ, mod in ('png', png), ('bmp', bmp), ('jpeg', jpeg):
      ...
      if hasattr(mod, "to_png"):
        ...
        ret = subprocess.run(['convert', '-', 'png:-'],
  ...
  def color(self, color):
    ...
    if isinstance(color, tuple):
      if len(color) > 3:
        opacity = color[3]
      color = "#" + "".join("%02X" % c for c in color[:3])
  ...
  def _encode_block(text):
    ...
    if text[0][0] == "_":  # subscript
      return '<tspan font-size="80%%" baseline-shift="-15%%">%s</tspan>' % innertext
    elif text[0][0] == "^":  # superscript
      return '<tspan font-size="80%%" baseline-shift="35%%">%s</tspan>' % innertext
    elif text[0][0] == "~":  # overbar
      return '<tspan text-decoration="overline">%s</tspan>' % innertext
    assert False  # regex is bad
  ...
  def __repr__(self):
    ...
    svg.append(' '.join(('<svg xmlns="http://www.w3.org/2000/svg"',
               ...
               'stroke-width="%s"' % Svg.tounit(Svg.THICKNESS['wire']),
